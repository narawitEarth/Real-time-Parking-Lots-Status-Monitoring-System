...
cursor = conn.cursor()
# cursor.execute("INSERT INTO park2(detect,time) VALUES (5,CURRENT_TIMESTAMP)")
# conn.commit()
...
#'http://192.168.43.150:58545/videostream.cgi?user=admin&pwd=TApop123'
    
    cap = cv2.VideoCapture("video.avi")
    #cap = cv2.VideoCapture('http://192.168.43.150:58545/videostream.cgi?user=admin&pwd=TApop123')
    ...
    while(cap.isOpened()):
        ...
        for j in list_of_classes:
            ...
            if(check != free): #Free Change
                ...
                cursor.execute("INSERT INTO park(detect,time) VALUES ({},CURRENT_TIMESTAMP)".format(free))
                print("Before Check: ",check)
                check = free
                print("After Check: ",check)
            cv2.putText(img, "Free Space: "+str(free), (300, 50) , cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 255, 0), lineType=cv2.LINE_AA)
